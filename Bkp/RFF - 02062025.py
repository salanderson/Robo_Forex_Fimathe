...
def converter_timeframe(valor):
    ...
    return mapa.get(valor.upper(), mt5.TIMEFRAME_M15)  # padrão M15
...
def carregar_config_ativos(caminho_arquivo):
    ...
    for linha in linhas:
        ...
        if not linha:
            if bloco:
                # Converte o timeframe textual para constante MT5
                if 'timeframe' in bloco:
                    bloco['timeframe_mt5'] = converter_timeframe(bloco['timeframe'])
                else:
                    bloco['timeframe_mt5'] = mt5.TIMEFRAME_M15  # padrão
                ativos.append(bloco)
                bloco = {}
            continue
